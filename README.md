...
![Statements](https://img.shields.io/badge/statements-99.14%25-brightgreen.svg?style=flat)
![Branches](https://img.shields.io/badge/branches-93.07%25-brightgreen.svg?style=flat)
![Functions](https://img.shields.io/badge/functions-100%25-brightgreen.svg?style=flat)
![Lines](https://img.shields.io/badge/lines-99.12%25-brightgreen.svg?style=flat)
...
| POST   | `/users/refresh`                | —               | No body - reads the `refreshToken` cookie. Rotates it and returns a new `accessToken`. `401` if the cookie is missing, unknown, expired, or already rotated out.                                                                                                                           |
...
- Authentication uses a short-lived (15 minute) JWT access token verified on every request to a protected route via the `isAuthenticated` middleware, plus a rotating refresh token delivered only in an `httpOnly` cookie (never in a JSON body) - each `POST /users/refresh` replaces it, so a previously issued refresh token stops working immediately once a newer one is minted.
